...
Usage:
  python3 plex_watchlist_to_ics.py --out ~/Downloads/plex_movies.ics
...
def start_of_week_sunday(dt: datetime) -> datetime:
    # Sunday as start of week (week is Sunday–Saturday)
...
def main():
    ...
    # Week is Sunday–Saturday (7 days), so we can schedule at most 7 events in one week.
    count = max(1, min(args.count, len(movies), 7))
    selected = random.sample(movies, count)

    today = datetime.now()
    sunday = start_of_week_sunday(today)
    if args.week == "next":
        sunday = sunday + timedelta(days=7)

    # choose distinct days within the Sunday–Saturday week
    ...
    for i, movie in enumerate(selected):
        day = day_indexes[i]
        start_dt = (sunday + timedelta(days=day)).replace(hour=start_h, minute=start_m, second=0, microsecond=0)
